            ...
            using (MemoryStream ms = new MemoryStream())
            {
                using (Image image = new Bitmap(width, height, PixelFormat.Format32bppArgb))
                {
                    using (Graphics gdi = Graphics.FromImage(image))
                    {
                        ...
                        {
                            gdi.FillRoundedRectangle(borderBrush, 0, 0, image.Width, image.Height, Radius);
                        }

                        using (Brush textBackgroundBrush = new SolidBrush(textBackColor))
                        {
                            gdi.FillRoundedRectangle(textBackgroundBrush, borderWidth, borderWidth,
                                                     (image.Width - (borderWidth*2) - 1), (image.Height - (borderWidth*2) - 1),
                                                     Radius);
                        }
                        ...
                        using (Font font = new Font(fontName, fontSize, FontStyle.Regular, GraphicsUnit.Pixel))
                        {
                            ...
                            {
                                SizeF textSize = gdi.MeasureString(Settings.PromoteTextForCounter, font);

                                float x = (((textWidth - textSize.Width) / 2) + borderWidth);
                                float y = ((image.Height - textSize.Height) / 2);

                                gdi.DrawString(Settings.PromoteTextForCounter, font, textForegroundBrush, x, y);
                            }

                            using (Pen countBackgroundPen = new Pen(countBackColor, 3.0f))
                            {
                                gdi.DrawRoundedRectangle(
                                    countBackgroundPen, (textWidth + borderWidth), borderWidth,
                                    (countSize.Width + CountWidthBuffer - 1),
                                    (image.Height - ((borderWidth + 0.5f) * 2)), Radius);
                            }

                            using (Brush countForegroundBrush = new SolidBrush(countForeColor))
                            {
                                using (Font counterFont = new Font(font, FontStyle.Bold))
                                {
                                    float x = ((((countSize.Width + CountWidthBuffer) - countSize.Width)/2) +
                                               borderWidth + textWidth);
                                    float y = ((image.Height - countSize.Height)/2);

                                    gdi.DrawString(count.ToString(Constants.CurrentCulture), counterFont, countForegroundBrush,
                                                   x, y);
                                }
                            }
                        }
