namespace Kigg.Infrastructure
{
    ...
    public class EmailSender : IEmailSender
    {
        ...
        public void SendNewPassword(string email, string userName, string password)
        {
            ...
            SendMailAsync(_settings.WebmasterEmail, email, "{0}: Twoje nowe hasło".FormatWith(_settings.SiteTitle), body);
        }
        ...
        public void SendComment(string url, IComment comment, IEnumerable<IUser> users)
        {
            ...
            ThreadPool.QueueUserWorkItem(state =>
                                                    {
                                                         foreach (IUser user in users.Where(u=>u.Email != comment.ByUser.Email))
                                                         {
        ...
        public void NotifySpamStory(string url, IStory story, string source)
        {
            ...
            SendMailAsync(_settings.WebmasterEmail, _settings.SupportEmail, "Opublikowany Spam artykuł - {0}".FormatWith(SystemTime.Now().ToLongDateString()), body);
        }
        ...
        public void NotifyStoryMarkedAsSpam(string url, IStory story, IUser byUser)
        {
            ...
            SendMailAsync(_settings.WebmasterEmail, _settings.SupportEmail, "Artykuł Spam - {0}".FormatWith(SystemTime.Now().ToLongDateString()), body);
        }
        ...
        public void NotifySpamComment(string url, IComment comment, string source)
        {
            ...
            SendMailAsync(_settings.WebmasterEmail, _settings.SupportEmail, "Komentarz Spam  - {0}".FormatWith(SystemTime.Now().ToLongDateString()), body);
        }
        ...
        public void NotifyStoryApprove(string url, IStory story, IUser byUser)
        {
            ...
            SendMailAsync(_settings.WebmasterEmail, _settings.SupportEmail, "Zaakceptowane artykuły - {0}".FormatWith(SystemTime.Now().ToLongDateString()), body);
        }
        ...
        public void NotifyConfirmSpamStory(string url, IStory story, IUser byUser)
        {
            ...
            SendMailAsync(_settings.WebmasterEmail, _settings.SupportEmail, "Artykuł zatwierdzony jako Spam - {0}".FormatWith(SystemTime.Now().ToLongDateString()), body);
        }
        ...
        public void NotifyCommentAsOffended(string url, IComment comment, IUser byUser)
        {
            ...
            SendMailAsync(_settings.WebmasterEmail, _settings.SupportEmail, "Komentarz oznaczony jako obraźliwy - {0}".FormatWith(SystemTime.Now().ToLongDateString()), body);
        }
        ...
        public void NotifyPublishedStories(DateTime timestamp, IEnumerable<PublishedStory> stories)
        {
            ...
            string subject = "{0}: Opublikowane artykuły - {1}".FormatWith(_settings.SiteTitle, timestamp.ToString());
            ...
            if (stories.Count() > 0)
            {
                ...
                bodyBuilder.Append(",\"Suma\",\"Ranga\",\"Kategoria\",\"Tytuł\",\"Url\"");
