    ...
    public static class HtmlHelperExtension
    {
                ...
                Action<string, string> addIcon = (type, url) =>
                                                 {
                                                     ...
                                                     html.Append(" <a href=\"{0}\" target=\"_blank\" class=\"pageHeader-cion\"><img alt=\"{1}\" title=\"{1}\" src=\"{2}\"/></a>".FormatWith(helper.AttributeEncode(url), type, helper.AttributeEncode(iconUrl)));
                                                 };
        ...
        public static string Pager(this HtmlHelper helper, string routeName, string actionName, string controllerName, IDictionary<string, object> values, string pageParamName, bool appendQueryString, int pageCount, int noOfPageToShow, int noOfPageInEdge, int currentPage)
        {
            ...
            if (pageCount > 1)
            {
                ...
                if (currentPage > 1)
                {
                    pagerHtml.Append(getPageLink("Poprzednia", currentPage - 1));
                }
                else
                {
                    pagerHtml.Append(" <span class=\"disabled\">Poprzednia</span>");
                }
                ...
                {
                    pagerHtml.Append(getPageLink("Nastêpna", currentPage + 1));
                }
                else
                {
                    pagerHtml.Append(" <span class=\"disabled\">Nastêpna</span>");
                }
