...
<div id="commentDisc"><span>Każdy z Użytkowników ponosi wyłączną odpowiedzialność wobec <%= Html.ActionLink("dotnetomaniak.pl", "Story", "Published")%> i osób trzecich za treść swoich wypowiedzi.</span>
</div>
<div id="commentList">
            ...
            <%{%>
                ...
                <li id="li-<%= Html.AttributeEncode(commentId)%>">
                    ...
                    <p class="meta <%= isOwner ? "metaOwner" : "metaVisitor" %>">
                        ...
                        <span class="time" title="<%=comment.CreatedAt.ToString("F")%> GMT"><%= comment.CreatedAt.ToRelative() %></span> temu napisał:
                    </p>
    ...
    <div class="form">
        <form id="frmCommentSubmit" action="<%= Url.Action("Post", "Comment") %>" method="post">
            ...
            <div>
                ...
                <div><label class="smallLabel"><input id="chkSubscribe" name="subscribe" type="checkbox" value="true" checked="checked"/>Subskrybuj przez email</label></div>
            </div>
...
<% {%>
    <div class="commentMessage">
        Aby umieścić komentarz <a id="lnkCommentLogin" class="actionLink" href="javascript:void(0)">zaloguj się</a> lub <a id="lnkCommentSignup" class="actionLink" href="javascript:void(0)">zarejestruj</a>
    </div>
<% }%>
